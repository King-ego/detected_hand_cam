...
def is_bsl_a(landmarks, w, h, thumb_index_thresh=0.12, finger_fold_thresh=0.12 ):
    try:
        ...
        palm_center = (
            (index_mcp[0] + middle_mcp[0] + ring_mcp[0] + pinky_mcp[0]) / 5.0,
            (index_mcp[1] + middle_mcp[1] + ring_mcp[1] + pinky_mcp[1]) / 5.0
        )
        ...
        for pip_idx, tip_idx in fingers:
            ...
            close_to_palm = distance(tip, palm_center) < finger_fold_thresh * min_side
            not_extended_along_hand = proj_along_hand(tip) <= proj_along_hand(pip) + finger_fold_thresh * min_side

            if not (close_to_palm or not_extended_along_hand):
                return False
